...
exempt_by_category = (
    fies_raw_vat_etr
    .with_columns([
        pl.sum_horizontal(col for col in exempt_items[name] if col in exempt_items]).alias(f'{name}_exempt')
        for name in expenditure_categories
    ])
    .group_by('NPCINC')
    .agg([
        pl.sum('RFACT').alias('number_households')] +
        [pl.sum(f'{name}_exempt') for name in expenditure_categories]
    )
    .sort('NPCINC')
)
